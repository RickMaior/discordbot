const {messageCollect} = require("./messageFunctions.js")

//Asks on dm what is webhook url and returns an objet with id and token
async function  webHookByURL(message) {
    const hook = await messageCollect("What is the webhook url",message)
...
async function  webHookByIdToken(message) {
    const hookID = await messageCollect("What is the webhook id",message)
    const hookToken = await messageCollect("What is the webhook token",message)
